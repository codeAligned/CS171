...
static GLint modeUniformPos;
int mode = 0; // start out in per-vertex mode
...
static void readShaders() {
   ...
   modeUniformPos = glGetUniformLocation(shaderProgram, "mode");
...
static void doList() {
   ...
   glUniform1f(modeUniformPos, mode);
...
{
    if (key == 'w' || key == 'w') {
        if (mode == 0) { // toggle "per-vertex" and "per-fragment" modes
          mode = 1;
        } else {
          mode = 0;
        }
...
int main(int argc, char *argv[]) {
   ...
   if (argc != 3) {
     std::cerr << "USAGE: glslRenderer xRes yRes" << std::endl;
     exit(1);
   }
